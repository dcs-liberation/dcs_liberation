...
def to_datetime(
    ...
    day_begin_utc.replace(tzinfo=ZoneInfo("UTC"))
    sun_info = sun.sun(location.observer, date=day_begin_utc.date())

    if night_disabled:
        hours_per_block = (
            (sun_info["dusk"] - sun_info["dawn"]) / datetime.timedelta(hours=1) / 4.0
        )
        ...
        if time_of_day == TimeOfDay.Dawn:
            ...
            begin = sun_info["dusk"]
            end = day_begin_utc + datetime.timedelta(days=1)
    begin_hour = round((begin - day_begin_utc).total_seconds() / 3600)
    end_hour = round((begin - day_begin_utc).total_seconds() / 3600)
